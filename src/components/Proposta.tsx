...
function Campo({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <p className="mb-3 text-[13px] font-semibold uppercase tracking-[0.16em] text-papel/45">{label}</p>
      {children}
    </div>
  )
}
      ...
      {options.map((o) => {
        ...
        return (
          <button key={o} role="radio" aria-checked={on} onClick={() => onChange(o)}
            className={`rounded-full border px-4 py-2.5 text-[15px] font-medium transition-all duration-200 ${on ? 'border-ouro bg-ouro text-tinta shadow-[0_8px_24px_-12px_rgba(232,178,60,.9)]' : 'border-papel/15 bg-papel/[.04] text-papel/70 hover:border-papel/35 hover:text-papel'}`}>
            {o}
          </button>
        )
      })}
...
export default function Proposta() {
  ...
  return (
    <Sheet id="proposta" tone="dark" glow>
      <Container>
        <Heading
          align="center" kicker="Solicitar proposta"
          title={<>Descubra quanto dá para <span className="italic text-ouro">antecipar hoje</span>.</>}
        ...
        <div className="mt-14 grid gap-8 lg:mt-20 lg:grid-cols-[minmax(0,7fr)_minmax(0,5fr)] lg:gap-12">
          <motion.div
            ...
            className="flex flex-col gap-8 rounded-[32px] border border-papel/[.1] bg-papel/[.03] p-7 sm:p-10"
          >
            <Campo label="Seu nome">
              <input value={nome} onChange={(e) => setNome(e.target.value)} placeholder="Como podemos te chamar?" aria-label="Seu nome" autoComplete="name"
                className="h-13 w-full rounded-2xl border border-papel/15 bg-papel/[.04] px-4 py-3.5 text-[16px] text-papel placeholder:text-papel/35 focus:border-ouro/60 focus:outline-none" />
            </Campo>
          ...
          <div className="lg:sticky lg:top-28 lg:self-start">
            <div className="overflow-hidden rounded-[32px] border border-papel/[.1] bg-[#0b141a] shadow-[0_40px_100px_-40px_rgba(0,0,0,.9)]">
              <div className="flex items-center gap-3 border-b border-papel/[.06] bg-[#1f2c34] px-5 py-3.5">
                ...
                <div className="leading-tight">
                  <p className="text-[15.5px] font-semibold text-papel">Rezende Precatórios</p>
                  <p className="text-[12.5px] text-papel/50">Prévia da sua mensagem</p>
                </div>
              </div>
              ...
              <div className="border-t border-papel/[.06] bg-[#111b21] p-4">
            ...
            <a href={`mailto:${EMAIL}`} className="mt-5 flex items-center justify-center gap-2.5 break-all text-[14.5px] text-papel/50 hover:text-ouro">
