...
function Item({ q, a, aberto, onToggle, id }: { q: string; a: ReactNode; aberto: boolean; onToggle: () => void; id: string }) {
  return (
    <div className={`rounded-[24px] border transition-colors duration-300 ${aberto ? 'border-ouro/35 bg-papel/[.05]' : 'border-papel/[.1]'}`}>
      <h3>
        <button onClick={onToggle} aria-expanded={aberto} aria-controls={id} className="flex w-full items-center justify-between gap-6 px-6 py-5 text-left sm:px-7 sm:py-6">
          <span className="font-display text-[19px] leading-snug text-papel sm:text-[22px]">{q}</span>
          <span className={`grid h-9 w-9 shrink-0 place-items-center rounded-full transition-all duration-300 ${aberto ? 'rotate-45 bg-ouro text-tinta' : 'bg-papel/[.08] text-ouro'}`}>
      ...
      <AnimatePresence initial={false}>
        {aberto && (
          <motion.div id={id} initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.4, ease: EASE }} className="overflow-hidden">
            <p className="px-6 pb-6 text-[16px] leading-relaxed text-papel/60 sm:px-7 sm:pb-7">{a}</p>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
    ...
    <Sheet id="duvidas" tone="dark">
      <Container className="grid gap-12 lg:grid-cols-[minmax(0,4fr)_minmax(0,7fr)] lg:gap-16">
        <div className="lg:sticky lg:top-28 lg:self-start">
          <Heading kicker="Dúvidas" title={<>Perguntas que <span className="italic text-ouro">todo mundo faz</span>.</>} lead="Se a sua não estiver aqui, pergunte direto. Respondemos sem enrolação." />
          <a href={waLink('Olá! Tenho uma dúvida sobre antecipação de precatório: ')} target="_blank" rel="noopener noreferrer"
            className="btn-linha-clara mt-8 inline-flex h-13 items-center gap-2.5 rounded-full px-6 py-3.5 text-[15.5px] font-semibold">
