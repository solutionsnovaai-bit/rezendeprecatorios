...
export default function Hero({ ready }: { ready: boolean }) {
  ...
  return (
    <section
      ...
      className="hero grao relative isolate min-h-[100svh] overflow-hidden bg-papel pb-28 pt-[104px] lg:flex lg:items-center lg:pt-[124px] lg:pb-20"
    >
      ...
      <motion.div className="hero-copy relative z-10 px-6 sm:px-10 lg:px-0" style={reduce ? undefined : { x: copyX }}>
        <motion.h1 {...item(0.08)} className="hero-title font-display font-medium leading-[1.02] tracking-[-0.025em] text-tinta">
          O dinheiro já é seu.
        ...
        <motion.ul {...item(0.4)} className="mt-10 flex flex-wrap items-center gap-x-7 gap-y-3 text-[14px] text-tinta/55">
          {SELOS.map((s) => (
            <li key={s} className="flex items-center gap-2.5 whitespace-nowrap">
              <span className="grid h-5 w-5 shrink-0 place-items-center rounded-full bg-marinho/10 text-marinho">
