...
export default function Espera() {
  ...
  return (
    <Sheet id="espera" tone="light" glow>
      <Container>
        <Heading
          tone="light"
          kicker="A fila do precatório"
          title={<>Você já esperou <span className="italic text-azul">tempo demais</span>.</>}
        ...
        <div ref={ref} className="relative mt-14 lg:mt-20">
          <div aria-hidden className="absolute left-[27px] top-3 bottom-10 w-px bg-tinta/10 lg:left-0 lg:right-0 lg:top-[27px] lg:h-px lg:w-auto lg:bottom-auto" />
          <motion.div aria-hidden style={{ scaleY: linha }} className="absolute left-[27px] top-3 bottom-10 w-px origin-top bg-gradient-to-b from-ceu to-azul lg:hidden" />
          <motion.div aria-hidden style={{ scaleX: linha }} className="absolute left-0 right-0 top-[27px] hidden h-px origin-left bg-gradient-to-r from-ceu via-azul to-marinho shadow-[0_0_16px_rgba(26,138,216,.5)] lg:block" />

          <ol className="relative grid gap-9 lg:grid-cols-4 lg:gap-6">
            {ETAPAS.map((e, i) => (
              <li key={e.titulo} className="relative flex gap-6 lg:block">
                <span className={`relative grid h-14 w-14 shrink-0 place-items-center rounded-full border font-display text-[19px] shadow-[0_0_0_8px_#faf7f1] ${i === 3 ? 'border-marinho bg-marinho text-papel' : 'border-tinta/15 bg-white text-tinta/60'}`}>
                  {i + 1}
                </span>
                <div className="pt-2 lg:pt-8">
                  <h3 className={`font-display text-[23px] leading-tight ${i === 3 ? 'text-marinho' : 'text-tinta'}`}>{e.titulo}</h3>
                  <p className="mt-2 max-w-[32ch] text-[15.5px] leading-relaxed text-tinta/60">{e.texto}</p>
                </div>
              </li>
            ))}
          </ol>
        ...
        <motion.div ref={cardRef} style={{ scale: escala }} className="mt-14 grid gap-4 lg:mt-20 lg:grid-cols-2">
          <div className="rounded-[30px] border border-tinta/10 bg-white p-8 sm:p-10">
            <p className="text-[12px] font-semibold uppercase tracking-[0.24em] text-tinta/40">Esperando na fila</p>
            <p className="mt-6 font-display text-[clamp(3rem,7vw,5rem)] leading-none text-tinta/85">
              {visivel ? 'anos' : 'anos'}
            </p>
            <p className="mt-5 max-w-[34ch] text-[16px] leading-relaxed text-tinta/60">
